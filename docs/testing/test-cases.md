...
- [テストケース一覧](#テストケース一覧)
  - [機能1: ノード管理](#機能1-ノード管理)
  - [機能2: ツリー表示とナビゲーション](#機能2-ツリー表示とナビゲーション)
  - [機能3: コピー&ペースト機能](#機能3-コピーペースト機能)
  - [機能4: 親ノード表示機能](#機能4-親ノード表示機能)
- [テスト環境](#テスト環境)
- [テスト実行方法](#テスト実行方法)
...
#### TC-FUNC-015: 作成者ドロップダウン選択
- **テスト名**: 作成者フィールドのドロップダウン選択機能テスト
- **前提条件**: ツリーに複数のメンバー（オーナー・編集者）が存在する
- **テストステップ**:
  1. ノードを選択
  2. 作成者フィールドがドロップダウンとして表示されることを確認
  3. ドロップダウンにオーナーと編集者のみが表示されることを確認（閲覧者は非表示）
  4. 別のメンバーを選択
  5. 保存ボタンをクリック
  6. 作成者が変更されていることを確認
- **期待結果**: 編集権限のあるメンバーのみがドロップダウンに表示され、作成者を変更できる

#### TC-FUNC-016: ノード新規作成時の作成者デフォルト値
- **テスト名**: ノード新規作成時に作成者がログインユーザーになることのテスト
- **前提条件**: ログイン済みのユーザーがいる
- **テストステップ**:
  1. 「ルートノード追加」または「子ノード追加」をクリック
  2. ノード作成ダイアログが表示される
  3. 作成者フィールドにログインユーザーがデフォルトで選択されていることを確認
  4. ノードを作成
  5. 作成されたノードの作成者がログインユーザーであることを確認
- **期待結果**: 新規ノードの作成者は自動的にログインユーザーが設定される
...
#### TC-FUNC-014: 保存ショートカット
- **テスト名**: 保存ショートカット機能のテスト
- **前提条件**: ノードが選択されている
- **テストステップ**:
  1. ノードを選択
  2. タイトルまたは内容を編集
  3. Mac: `Cmd + Enter` / Windows: `Ctrl + Enter` を押す
  4. ノードが保存されることを確認
  5. 保存ボタンにマウスオーバーしてツールチップを確認
- **期待結果**: ショートカットで保存が実行され、ツールチップにショートカット情報が表示される
