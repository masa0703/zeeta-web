...
**権限**:
- Owner: 招待送信、役割変更、メンバー削除 可能
- Editor: 招待送信 可能、役割変更・削除 不可
...
| フィールド | ID | スタイル | 備考 |
|-----------|-----|----------|------|
| タイトル | `#node-title` | `text-lg font-medium` | 大きめフォント |
| 作成者 | `#node-author` | select | 編集権限者のみリスト表示 |
| 内容 | `#node-content` | textarea (10行) | Markdown対応 |
...
| ボタン | ID | 色 | ショートカット |
|--------|-----|-----|--------------|
| 保存 | `#save-node-btn` | 緑 | `Cmd/Ctrl + Enter` |
| 削除 | `#delete-node-btn` | 赤 | - |
| 子ノード追加 | `#add-child-node-btn` | 青 | - |

**ツールチップ**: 保存ボタンにホバーで "保存 (Cmd+Enter / Ctrl+Enter)" 表示
